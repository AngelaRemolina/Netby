...
router.get('/dashboard/capture', isLoggedIn, async (req, res) => {
    ...
    try {
        if (fs.existsSync(path)) {
            ...
            var command = "rm " + path;
            const childRemove = exec(command);
            //fs.unlinkSync(path);
...
router.get('/waitpage', isLoggedIn, async (req, res) => {
    res.render('dashboard/captures/waitpage');
    req.flash('sleeptime', 'Espere');
    const path = './capture.json'
    try {
        if (fs.existsSync(path)) {
            console.log("existoooooooooooooooooooo")
            res.redirect('dashboard/capture');
        } else {
            setTimeout(res.redirect(req.get('referer')), 30000);
        }
    } catch (err) {
        console.error(err)
        req.flash('message', 'An error ocurred during the capture.');
